...
class GA():
    def __init__(self, nDenseBlock, Bottleneck):
        ...
        self.num_crossover = arg.number_crossover
        self.num_mutation = arg.number_mutation
        ...
        utils.print_and_log(logger,
                            "number_mutation : " + str(arg.number_mutation)+ " number_crossover = " + str(arg.number_crossover) + " rs or ga? =" + str(arg.mode))
    ...
    def row_crossover(self, p1, p2,new_population, nDenseBlocks):
    ...
    def col_crossover(self, p1, p2,new_population, nDenseBlocks):
        ...
        offspring2 = np.concatenate((parent2[:, :point, :], parent1[:, point:, :]),
                                    axis=1)
        return offspring1, offspring2
    def both_crossover(self, p1, p2,new_population, nDenseBlocks):
        parent1 = np.array(new_population[p1 * self.number_blocks:(p1 + 1) * self.number_blocks])
        parent2 = np.array(new_population[p2 * self.number_blocks:(p2 + 1) * self.number_blocks])
    ...
    def crossover(self, p1, p2, new_population, nDenseBlocks):
        if self.num_crossover == 3:
            ran_num = np.random.rand()
            if ran_num <= 0.33:
                offspring1, offspring2 = self.row_crossover(p1, p2, new_population, nDenseBlocks)
            elif 0.33 < ran_num and ran_num < 0.66:
                offspring1, offspring2 = self.col_crossover(p1, p2, new_population, nDenseBlocks)
            else:
                offspring1, offspring2 = self.both_crossover(p1, p2, new_population, nDenseBlocks)
        elif self.num_crossover == 2:
            ran_num = np.random.rand()
            if ran_num <= 0.5:
                offspring1, offspring2 = self.row_crossover(p1, p2, new_population, nDenseBlocks)
            elif 0.5 < ran_num:
        ...
        return offspring1, offspring2
    def bitflip_mutation(self, offspring1, offspring2, nDenseBlocks):
        ...
        return off1, off2
    def rowflip_mutation(self, offspring1, offspring2, nDenseBlocks):
        # bit-flipping row
        off1 = np.array(offspring1)
        off2 = np.array(offspring2)
        ...
        return off1, off2
    def colflip_mutation(self, offspring1, offspring2, nDenseBlocks):
        # bit-flipping col
        off1 = np.array(offspring1)
        off2 = np.array(offspring2)
    ...
    def mutation(self, offspring1, offspring2, nDenseBlocks):
        if self.num_mutation == 3:
            ran_num = np.random.rand()
            if ran_num <= 0.33:
                off1, off2 = self.bitflip_mutation(offspring1, offspring2, nDenseBlocks)
            elif 0.33 < ran_num and ran_num < 0.66:
                off1, off2 = self.rowflip_mutation(offspring1, offspring2, nDenseBlocks)
            else:
                off1, off2 = self.colflip_mutation(offspring1, offspring2, nDenseBlocks)
        elif self.num_mutation == 2:
            ran_num = np.random.rand()
            if ran_num <= 0.5:
                off1, off2 = self.bitflip_mutation(offspring1, offspring2, nDenseBlocks)
            elif 0.5 < ran_num:
        ...
        if arg.mode == 'ga':
            ...
            for generation in range(self.generations):
                ...
                for n in range(len(new_population)//3//2):
                    ...
                    cr_rand = np.random.rand()
                    if cr_rand <= arg.crossover_rate:
                ...
                if len(offspring_population) < 96:
                    repair = 96-len(offspring_population)
                    parents_population = parents_population_rank[:int((self.pop_size * elite_rate)+repair//3) * self.number_blocks]
                    parents_fitness = parents_fitness[:int((self.pop_size * elite_rate)+repair//3)]
                    idx_parents = idx_parents_rank[:int((self.pop_size * elite_rate)+repair//3) * self.number_blocks]
                    parents_acc = parents_acc[:int((self.pop_size * elite_rate)+repair//3)]
            ...
            for generation in range(self.generations):
                ...
                new_population = self.create_init_pop(nDenseBlocks)
                for p in range(self.pop_size * 3):
                    idx.append(self.chk(nDenseBlocks, new_population[p]))

                do_GAtrain = int(self.pop_size * train_rate)
                if arg.surrogate == "True":
                    for m in range(1, len(new_population) // 3):
                        matrix = np.array(new_population[(m - 1) * self.number_blocks:m * self.number_blocks])
                        test_matrix = matrix.ravel().reshape(1, -1)
                        accuracy = predictor.predict(test_matrix)
                        acc.append(accuracy[0])
                    ...
                    top4_chd = [index for index, value in top4_chd[:do_GAtrain]]
                    add_label = []
                    # for i in range(do_GAtrain):
                    #     net = DenseNet(growthRate=arg.growthRate, depth=arg.depth, reduction=arg.reduction,
                    #                    bottleneck=arg.bottleneck, nClasses=arg.nClasses,
                    #                    matrix=new_population[((self.pop_size + top4_chd[do]) - 1) * self.number_blocks:(self.pop_size + top4_chd[do]) * self.number_blocks],
                    #                    idx=idx[(i) * self.number_blocks:i * self.number_blocks]).to(
                    #         device=arg.device)
                    # print("top4_chd = ", top4_chd)
                    for do in range(do_GAtrain):
                        # print("top4_chd = ", ((top4_chd[do]) - 1))
                        net = DenseNet(growthRate=arg.growthRate, depth=arg.depth, reduction=arg.reduction,
                                       bottleneck=arg.bottleneck, nClasses=arg.nClasses,
                                       matrix=new_population[((top4_chd[do]) - 1) * self.number_blocks:(top4_chd[do]) * self.number_blocks],
                                       idx=idx[((top4_chd[do]) - 1) * self.number_blocks:(top4_chd[do]) * self.number_blocks]).to(
                            device=arg.device)
                        torch.cuda.empty_cache()
                        net, loss = GAtrain(net, trainloader, arg.GA_epoch, arg.device)
                        acc[top4_chd[do]] = loss
                        add_label.append(loss)

                        matrix = np.array(new_population[((top4_chd[do]) - 1) * self.number_blocks:(top4_chd[do]) * self.number_blocks])
                        new_train_matrix = matrix.ravel()
                        dataset.append(new_train_matrix)
